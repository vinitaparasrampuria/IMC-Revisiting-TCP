...
if not os.path.isfile(csv_filename):
    with open(csv_filename, 'a', newline='') as csvfile:
      ...
      header = 'port', 'mean_rtt', 'bandwidth_port', 'port_retrans2', 'cwn_half_port', 'packet_loss2', 'x2', 'cwnd_half_rate1','x3','ratio2'
...
if not os.path.isfile(output_filename):
    with open(output_filename, 'a', newline='') as csvfile:
      writer = csv.writer(csvfile)
      header = 'total_ports', 'ports', 'sum(y_values)', 'total_cwnd_half', 'total_retransmission2', 'total_retransmission2/total_cwnd_half', 'np.nanmean(list_ratio2)', 'reg_simple2.intercept_', 'reg_simple2.coef_[0]', 'reg_simple3.intercept_', 'reg_simple3.coef_[0]', 'router_dropped', 'router_sent', 'router_dropped/total_cwnd_half'
      writer.writerow(header)


for i in range (0,10):
  dat_cwn = pd.read_csv("/local/repository/cloudlab-scripts/result-"+cca1+"/sender-cwn-10.10.2.1"+str(i)+"-file.txt", header=None, names=['port', 'cwnd','rtt'])
...
with open(output_filename, 'a', newline='') as csvfile:
  writer = csv.writer(csvfile)
  columns = sum(ports), (sum(ports)-(num_clients*sender)), sum(y_values), total_cwnd_half, total_retransmission2, total_retransmission2/total_cwnd_half, np.nanmean(list_ratio2), reg_simple2.intercept_, reg_simple2.coef_[0], reg_simple3.intercept_, reg_simple3.coef_[0], dropped, sent, dropped/total_cwnd_half
  writer.writerow(columns)
