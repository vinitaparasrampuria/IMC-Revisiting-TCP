...
output_filename="data-cwn-"+ip+"-file.txt"

with open("sender-cwn1-"+ip+"-file.txt", 'r') as file:
    for line1,line2 in itertools.zip_longest(*[file]*2):
       
      ip1_match = re.search(r"10\.10\.1\.10:([\d]+)", line1)
      if ip1_match:
          value_after_ip1 = int(ip1_match.group(1))
          print(f"Value after 10.10.1.10: {value_after_ip1}")

      ip2_match = re.search(r"10\.10\.2\.10:([\d]+)", line1)
      if ip2_match:
